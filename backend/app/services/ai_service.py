...
from app.core.http import get_client # [핵심] 싱글톤 클라이언트 import
...
# [Retry 설정] 429, 5xx 에러 시 지수 백오프 (2초 -> 4초 -> 8초 ...)
@retry(
...
async def _call_gemini_api(contents_payload):
...
async def generate_summary(content: str, image_urls: list = None) -> str:
    ...
    # (1) 이미지 처리: 싱글톤 클라이언트 사용으로 최적화
    if image_urls and len(image_urls) > 0:
        target_url = image_urls[0]
        client = get_client() # [변경] 싱글톤 클라이언트 호출
        
        try:
            logger.info(f"🖼️ [Gemini] 이미지 다운로드: {target_url}")
            # 이미지는 외부 링크일 수 있으니 verify=False 유지 여부 주의 (학교 서버면 False 필요)
